    ...
    for( const prefix of keys ) {
        ...
        for( const fileDetails of allFiles ) {
            ...
            if(fileDetails.subPrefix !== ''){
                ...
                const subFolderTests = `${subFolder}/__tests__`;

                createIfDoesntExist(subFolder)
       
                if(fileDetails.isTest){
                    createIfDoesntExist(subFolderTests)
                    newLocation = `${subFolderTests}/${fileDetails.fileName}`;
                }
                else{
                    createIfDoesntExist(subFolderSource)
                    newLocation = `${subFolderSource}/${fileDetails.fileName}`;
                }
            }
            else{

                const sourceFolder = `${domainFolder}/src`;
                const testFolder = `${domainFolder}/__tests__`;
